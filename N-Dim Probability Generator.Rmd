...
\\The support of the Dirichlet distribution is the standard $k-1$ simplex. Which is given by $\Delta_{k-1} =\Big{\{}(p_1, ..., p_n)\in\mathbb{R}^n: \forall_{j\in\{1, ..., n\}}0\leq p_j\leq 1,\sum_{j=1}^n p_j = 1\Big{\}}$.
\\Furthermore the density can be given by:
\[f_{p_1, ..., p_n} (p_1, ..., p_n) =\frac{1}{B(\alpha)}\prod_{j=1}^n x_j^{\alpha_j-1}\;\;\mbox{for}\;\;(p_1, ..., p_n)\in\Delta_{k-1}\]
...
\[=\frac{\Gamma (n)}{(\Gamma(1))^n} =\frac{(n-1)!}{(0!)^n} = (n-1)!\;\;\mbox{for}\;\;(p_1, ..., p_n)\in\Delta_{k-1}\]
